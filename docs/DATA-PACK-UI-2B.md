...
{
  "duration_ticks": {
    "bind": "personal/pixel_tzz:current_task_elapsed"
  },
  "fallback": {"text": "时间未公开"}
}
```

`duration_ticks` 只接受整数。权威值和协议仍使用 Tick，页面显示按 20 Tick/秒向下取整：

- 不足一小时为 `MM:SS`，例如 `535` Tick 显示 `00:26`；
- 一小时及以上为 `H:MM:SS`，例如 `74460` Tick 显示 `1:02:03`；
- 负数按 `00:00` 显示；
- 缺失或类型错误时使用显式 `fallback`，不会把内部值直接暴露给玩家。
...
卡片样式的 `normal.frame_style` 可以使用：

- `plain`、`rail`：基础平面与信息轨；
- `dossier`：切角档案框；
- `target`：目标锁定框；
- `broadcast`：带扫描横纹的广播面板；
- `warning`：双层警示框；
- `portal`：适合玩家终端入口的连接式导航片，使用短信号轨与切角表达可进入内容；
- `manual`：资源包无关的实体文档封面，模组绘制封面、书脊、内页与书签，数据包仍负责颜色、标题和内部排版。

`portal` 与 `manual` 是通用视觉原语，不绑定页面 ID。数据包可以在任意合法 Card
上引用它们；不得依赖示例数据包中的具体样式名。

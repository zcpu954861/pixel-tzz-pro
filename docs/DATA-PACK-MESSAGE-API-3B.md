...
状态：**资源格式与编译契约已冻结；网络协议已升至 v12；运行时集成和 Minecraft 实机验收仍在进行**
...
- `text_effects`、`message_cues` 的资源格式、默认值、引用与隔离契约已冻结；
- 三种动态字段捕获、光标覆盖色、新字覆盖色和瞬切/渐变回色已经进入 Schema 与纯逻辑自检；
- 编译器已经能生成独立消息目录，保留有界诊断并阻断无降级的 required cue；
- v12 payload、播放计划 codec 和有界客户端收件箱已经存在；
- `final_only` 能力/资产握手已有固定 100 Tick 上限；最终投影撤权会使用更高 cycle 的同实例
  `REFRESH` 或无文本 `CANCEL`，静态降级与冷启动投递均在实际发送边界 fail-closed；
- 客户端 Chat/HUD 播放、完整效果、偏好入口和默认未绑定的手动补全按键已经接线，但尚未完成
  Minecraft 多客户端实机验收；
- 服务端调用桥、实例控制、目录、审阅和无副作用主持人预览已经接线，仍待 Minecraft 多客户端
  实机验收；
- Chat Heads 和其他聊天界面模组尚未形成可宣称通过的实机兼容结论。

因此 V3B 目前不能标记为完成，也不能据此提交、推送或跳过用户逐项实机验收。

...
/** Bounded identity header and document codec used only by the V3C countdown surface. */
final class CountdownFrameCodec {
	static final int MAX_REPLACE_BYTES = 65_536;
	static final int MAX_PATCH_BYTES = 16_384;

	private CountdownFrameCodec() {
	}
	...
	static Header requireHeader(
		...
		return new Header(gameInstanceId, definitionGeneration, epoch, contextVersion, sequence);
	}
	...
	static byte[] readDocument(
		...
		return requireDocument(readByteArray(buffer, maximumBytes, field), maximumBytes, field);
	}
	...
	record Header(
		...
		Header {
			...
			if (
				...
				throw new IllegalArgumentException("countdown frame versions must be non-negative");
			}
		}
	}
}

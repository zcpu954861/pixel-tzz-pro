...
public final class CountdownPersistenceSelfCheck {
	...
	private static final UUID FORMER_HOST_ID = UUID.fromString(
		"00000000-0000-0000-0000-000000003c77"
	);
	...
	private static void checkCodecAndRecoveryRoundTrip() {
		...
		JsonElement legacyEncodedRoot = encodedRoot.deepCopy();
		legacyEncodedRoot.getAsJsonObject().remove("countdown_host_audience_ids");
		WorldStateV5 legacyDecodedRoot = WorldStateV5.CODEC
			.parse(JsonOps.INSTANCE, legacyEncodedRoot)
			.getOrThrow();
		check(
			legacyDecodedRoot.countdownHostAudienceIds().isEmpty(),
			"older schema-v5 roots must default the retained host audience to empty"
		);
		check(
			root.withCountdown(Optional.empty()).countdownHostAudienceIds().isEmpty(),
			"retiring a countdown must clear its retained host audience"
		);
	}
	...
	private static void checkWrapperCheckpointAndScopeRotation() {
		...
		var retainedHost = wrapper.commitV5(
			wrapper.stateRevision(),
			value -> value.retainCountdownHostAudience(Optional.of(FORMER_HOST_ID))
		);
		check(retainedHost.committed(), "former legal host audience must persist atomically");
		check(
			wrapper.currentV5().orElseThrow().countdownHostAudienceIds().equals(
				List.of(FORMER_HOST_ID)
			),
			"former legal host must be retained only in the countdown display audience"
		);
		...
		Instance resumed = CountdownAuthority.resumeRecovery(
			recovery,
			Set.of(
		...
		check(
			wrapper.currentV5().orElseThrow().countdown().isEmpty()
				&& wrapper.currentV5().orElseThrow().countdownHostAudienceIds().isEmpty(),
			"game-scope rotation must clear the old countdown and retained host audience"
		);
	}
